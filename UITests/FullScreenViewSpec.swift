    ...
    override func spec() {
        ...
        describe("Full-screen campaign view") {
            ...
            context("when clicking X button") {
                ...
                beforeEach {
                    launchAppIfNecessary(args: "-campaignType full-text-only")
                    if !iamView.exists {
            ...
            context("without controls") {
                ...
                it("should not display opt-out checkbox") {
                    expect(iamView.buttons["Do not show me this message again"].exists).to(beFalse())
                }
            }
            ...
            context("with action butttons") {
                ...
                beforeEach {
                    launchAppIfNecessary(args: "-campaignType full-controls")
                    if !iamView.exists {
                ...
                afterEach {
                    app.activate() // returns from the browser
                    usleep(250000) // wait for app switch animation to finish
                }
                ...
                it("should redirect to url after tapping button 1") {
                    iamView.buttons["Button0"].tap()
                    expect(XCUIApplication(bundleIdentifier: "com.apple.mobilesafari").state).toEventually(equal(.runningForeground))
                }
            ...
            context("with text with image layout") {
                ...
                beforeEach {
                    launchAppIfNecessary(args: "-campaignType full-text-image")
                    if !iamView.exists {
                        app.buttons["login_successful"].tap()
                        expect(iamView.exists).toEventually(beTrue())
                    }
            ...
            context("with image-only layout") {
                ...
                beforeEach {
                    launchAppIfNecessary(args: "-campaignType full-image-only")
                    if !iamView.exists {
                        app.buttons["login_successful"].tap()
                        expect(iamView.exists).toEventually(beTrue())
                    }
